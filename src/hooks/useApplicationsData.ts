...
interface UseApplicationsReturn {
  ...
  addApplication: (
    application: Omit<Application, "id" | "status" | "dateApplied">
  ) => Promise<void>;
...
export const useApplicationsData = (): UseApplicationsReturn => {
    ...
    try {
      ...
      if (
        ...
        if (result.jobMateData) {
          ...
          setWeeklyGoal(
            result.jobMateData.weeklyGoal || { current: 0, target: 10 }
          );
        }
      } else {
  ...
  const saveData = useCallback(
    async (newApplications: Application[], newWeeklyGoal: WeeklyGoal) => {
      try {
        if (
          typeof chrome !== "undefined" &&
          chrome.storage &&
  ...
  const addApplication = useCallback(
    async (
      applicationData: Omit<Application, "id" | "status" | "dateApplied">
    ) => {
      try {
  ...
  const updateApplicationStatus = useCallback(
    async (id: number, status: Application["status"]) => {
      try {
  ...
  const updateWeeklyGoal = useCallback(
    async (target: number) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to update weekly goal"
        );
      }
    },
    [applications, weeklyGoal, saveData]
