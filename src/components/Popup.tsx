...
const BottomNavigation: React.FC<{ activeTab: string; setActiveTab: (tab: string) => void; openDashboard: () => void }> = ({ activeTab, setActiveTab, openDashboard }) => (
  <div className="flex bg-white border-t border-gray-100 flex-shrink-0">
    {[
      { key: 'home', icon: Home, label: 'Home' },
      { key: 'applications', icon: BarChart, label: 'Applications' },
      { key: 'dashboard', icon: BarChart3, label: 'Dashboard', action: openDashboard },
...
const Popup: React.FC = () => {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
        ...
        if (result.jobMateData) {
          setData({
            applications: result.jobMateData.applications || getMockApplications(),
            weeklyGoal: result.jobMateData.weeklyGoal || { current: 5, target: 10 },
            currentProfile: result.jobMateData.currentProfile || 'product-manager'
          });
          return;
        }
      }
  ...
  const handleAutoFill = async () => {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.tabs && chrome.tabs.query) {
        ...
        showNotification('Auto-fill is only available in Chrome extension mode', 'warning');
      }
    } catch (error) {
      ...
      if (typeof chrome !== 'undefined' && chrome.tabs && chrome.tabs.query) {
        ...
        if (response) {
          const newApp: Application = {
            ...
            source: response.source.includes('linkedin') ? 'linkedin' : 
                   response.source.includes('indeed') ? 'indeed' :
                   response.source.includes('greenhouse') ? 'greenhouse' :
                   response.source.includes('lever') ? 'lever' : 'other',
            status: 'applied',
            dateApplied: new Date().toISOString(),
            url: response.url
          };
        ...
        showNotification('Application tracking is only available in Chrome extension mode', 'warning');
      }
    } catch (error) {
  ...
  const getCompanyLogo = (company: string): React.ReactNode => {
    const logoMap: Record<string, React.ReactNode> = {
      'Narvar': <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center text-white text-sm font-bold">N</div>,
      'Stripe': <div className="w-10 h-10 rounded-lg bg-purple-600 flex items-center justify-center text-white text-sm font-bold">S</div>,
      'Slack': <div className="w-10 h-10 rounded-lg bg-green-500 flex items-center justify-center text-white text-sm font-bold">⚡</div>,
      'AirBnB': <div className="w-10 h-10 rounded-lg bg-red-500 flex items-center justify-center text-white text-sm font-bold">A</div>,
      'X': <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center text-white text-sm font-bold">X</div>,
      'Google': <div className="w-10 h-10 rounded-lg bg-white border flex items-center justify-center text-sm font-bold">G</div>,
      'Mimi': <div className="w-10 h-10 rounded-lg bg-yellow-500 flex items-center justify-center text-white text-sm font-bold">M</div>,
      'Figma': <div className="w-10 h-10 rounded-lg bg-purple-500 flex items-center justify-center text-white text-sm font-bold">F</div>,
      'Apple': <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center text-white text-sm font-bold">🍎</div>
    };
    return logoMap[company] || <div className="w-10 h-10 rounded-lg bg-gray-500 flex items-center justify-center text-white text-sm font-bold">{company.charAt(0)}</div>;
  };
  ...
  const goalPercentage = Math.round((data.weeklyGoal.current / data.weeklyGoal.target) * 100);
  ...
  const getFilterCounts = () => {
    return {
      ...
      interviewing: data.applications.filter(app => app.status === 'interviewing').length,
      ...
      ghosted: data.applications.filter(app => app.status === 'ghosted').length
    };
  };
      ...
      <div className="w-[560px] h-[890px] bg-gray-50 overflow-hidden flex flex-col">
        ...
        <div className="flex justify-between items-center px-6 py-4 bg-white border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-xl text-gray-900">JobMate AI</span>
          </div>
          <div className="flex items-center gap-4">
            <div className="relative">
              <select 
                value={data.currentProfile}
                onChange={(e) => setData({...data, currentProfile: e.target.value})}
        ...
        <div className="px-6 py-6 bg-gray-50 flex-shrink-0">
          ...
          <div className="flex gap-3 mb-6 overflow-x-auto">
            {[
              ...
              { key: 'interviewing', label: `Interviewing (${filterCounts.interviewing})` },
        ...
        <div className="px-6 flex-1 overflow-y-auto bg-gray-50 min-h-0">
          <div className="space-y-4 pb-4">
            {filteredApplications.map((app) => (
              <div key={app.id} className="bg-white rounded-2xl p-4 hover:shadow-sm transition-all border border-gray-100">
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative flex-shrink-0">
                    <select
                      ...
                      className={`appearance-none px-4 py-2 pr-8 rounded-xl text-sm font-medium border cursor-pointer ${getStatusColor(app.status)} focus:outline-none`}
                    >
        ...
        <BottomNavigation activeTab={activeTab} setActiveTab={setActiveTab} openDashboard={openDashboard} />
      </div>
    );
  }

  return (
    <div className="w-[560px] h-[800px] bg-gray-50 overflow-hidden flex flex-col">
      {/* Header */}
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M6 0.0833334H12V4.52777V8.97223H6V13.4167H0V8.97223V4.52777H6V0.0833334Z" fill="currentColor" class="fill-[#0D141C] dark:fill-[#E3E3E3]"/>
</svg>

       
          <span className="font-semibold text-xl text-gray-900 dark:text-[#E3E3E3]">JobMate AI</span>
        </div>
        <div className="flex items-center gap-2 ">
          <div className="relative">
            <select 
          ...
          <div className="mb-2">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6 bg-white dark:bg-[#1F2937] border dark:border-[#374151]  rounded-[10px] px-3 py-4">
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Auto Fill Form</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Generate Cover Letter</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Analyze Job Fit</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Track This Application</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Tailor Resume</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Generate Answer</span>
              </button>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 mb-6 shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-3">
                <span className="text-sm text-gray-600">Weekly Goal: {data.weeklyGoal.current}/{data.weeklyGoal.target} jobs</span>
                <span className="text-sm font-semibold text-blue-600">{goalPercentage}%</span>
              </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-medium text-gray-900">Recent applications</h3>
              <button 
