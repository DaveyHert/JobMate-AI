      ...
      {THEME_OPTIONS.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className={`focus-visible:ring-ring relative flex h-8 w-10 items-center justify-center rounded-full transition-colors duration-200 outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${isActive ? "text-app-foreground" : "text-brand-muted-app-foreground hover:text-app-foreground"} `}
          >
            {isActive && (
              <motion.div
